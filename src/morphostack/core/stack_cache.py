...
class TrackingCacheKey:
    """Identity for a seeded tracking chain.

    Scoped so a cached track is never reused across a different source,
    seed, ROI/crop, Z-range, or spatial stack shape. Intentionally omits the
    UI threshold: the exact seeded tracker uses adaptive local thresholds,
    not the preview slider value.
    """
...
class TrackingResultCache:
    """LRU cache for ``SeededSliceResult`` lists from seeded Z tracking.

    Keyed by source identity, seed, ROI/Z context, and spatial shape. Stores
    the full ``list[SeededSliceResult]`` indexed by local frame. On subsequent
    preview requests, callers can reuse cached ok frames and resume from the
    furthest-reached frame instead of re-walking from the seed.
    """
